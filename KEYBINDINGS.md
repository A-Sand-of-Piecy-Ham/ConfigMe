...
**The whole bar goes grey** while the outer tmux is listening to nothing. It
pairs with `#h` in `status-right`: the outer bar keeps showing this host, the
inner one shows the server.

Setting `status-style` alone does nothing visible: `status-left`, the window
list and `status-right` each hard-code their colours with `#[...]`, so it only
repaints the gaps between them. Every drawn segment is overridden on toggle and
restored with `set -u`.
