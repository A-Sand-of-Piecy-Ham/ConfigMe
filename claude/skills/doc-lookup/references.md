...
astrocore v3.1 breaks keymaps on Neovim 0.12-dev | pin astrocore to c98c395 (v3.1.0) | 2026-09-17 | `invalid key: buf` from vim/keymap.lua on any FileType with treesitter textobjects. astrocore commit adc25f7 "align compatibility with Neovim 0.13" added `local buffer_key = vim.fn.has "nvim-0.12" == 1 and "buf" or "buffer"`, but the rename is a 0.13 change while the gate tests 0.12 -- so a 0.12-dev nightly picks "buf" and vim.keymap.set rejects it. Verified: `{buf=0}` errors, `{buffer=0}` works. Not fixed on origin/main as of this date. Both pins are v3.x so AstroNvim's `^3` is satisfied either way. Revisit when upstream corrects the gate.
